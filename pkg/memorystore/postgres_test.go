...
func TestPutMetadata(t *testing.T) {
	...
	metadataToMerge := []models.MessageMetadata{
		...
		},
		{
			...
			Key:      "some_top_level_key.new_key",
	...
	expectedMetadata := []models.Message{
		...
		},
		{
			Metadata: map[string]interface{}{
				...
				"some_top_level_key": map[string]interface{}{
	...
	err = putMessageMetadata(testCtx, &models.AppState{}, testDB, sessionID, metadataToMerge, true)
	...
	// Navigate through the metadata map in the retrieved testMessages
	for i, msg := range msgs {
		assert.Equal(t, expectedMetadata[i].Metadata, msg.Metadata)
	}
}
