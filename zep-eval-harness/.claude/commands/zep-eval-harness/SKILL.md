...
Watch three counters before trusting a tool run: `tests_hitting_call_cap`/`tests_hitting_iteration_cap` (the budget, not Zep, is bounding retrieval); `tests_with_no_calls` (nothing was retrieved — but it counts tests with no *executed* calls, so check `invalid_calls` in case the model only ever named a tool that doesn't exist); `tests_forced_answer_failed` (the agent never answered cleanly — it kept asking for tools, so the graded text is a preamble); `tests_require_tool_call_unenforced` (`--require-tool-call` was asked for but not applied — the provider rejected it or answered without retrieving); and `tool_choice_downgrades` (any rejected `tool_choice`, including on the forced-answer turn).
