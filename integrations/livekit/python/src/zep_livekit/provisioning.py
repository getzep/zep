...
``ZepUserAgent``'s lazy call into these helpers (see
``ZepUserAgent._ensure_resources``) is hot-path-wrapped and will never raise
into ``on_user_turn_completed``. Callers who want provisioning failures (and
