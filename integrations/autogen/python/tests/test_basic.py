...
class TestZepMemoryMock:
    ...
    @pytest.mark.asyncio
    async def test_update_context_calls_get_user_context_without_mode(self):
        """Context retrieval must not pass the removed V3 'mode' argument."""
        try:
            from zep_cloud.client import AsyncZep
        except ImportError:
            pytest.skip("zep_cloud not available")
        ...
        mock_client.thread.get = AsyncMock(return_value=MagicMock(messages=[]))
        ...
        mock_client.thread.get_user_context.assert_called_once()
        _, kwargs = mock_client.thread.get_user_context.call_args
        assert "mode" not in kwargs
        assert kwargs["thread_id"] == "test-session"
        assert "template_id" not in kwargs

    @pytest.mark.asyncio
    async def test_update_context_passes_template_id(self):
        """When configured, context_template_id is forwarded as template_id."""
        try:
            from zep_cloud.client import AsyncZep
        except ImportError:
            pytest.skip("zep_cloud not available")
        ...
        memory = ZepUserMemory(
            ...
            user_id="test-user",
            thread_id="test-session",
            context_template_id="customer-support",
        )
        ...
        _, kwargs = mock_client.thread.get_user_context.call_args
        assert "mode" not in kwargs
        assert kwargs["template_id"] == "customer-support"
