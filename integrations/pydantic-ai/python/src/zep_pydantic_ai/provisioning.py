...
The history processor's lazy call into these helpers (see
``zep_pydantic_ai.deps.ensure_user_and_thread``) is hot-path-wrapped and will
never raise into an agent run. Callers who want provisioning failures (and
