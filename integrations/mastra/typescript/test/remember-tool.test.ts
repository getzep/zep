...
describe("createZepRememberTool", () => {
  ...
  it("persists non-conversational data via graph.episode.add", async () => {
    ...
    expect(zep.graph.episode.add).toHaveBeenCalledWith("g1", {
      type: "text",
  ...
  it("routes to graph.episode.add for a standalone graph binding", async () => {
    const zep = makeFakeZep();
    const tool = createZepRememberTool({
      client: asZep(zep),
      binding: { graphUuid: "kb-1" },
    });

    // Even with a role, no thread means graph.episode.add is used.
    await run(tool, { content: "Refunds take 5 days", role: "assistant" });
    expect(zep.graph.episode.add).toHaveBeenCalledWith("kb-1", {
      type: "text",
  ...
  it("truncates over-long episode data to the 9900-char limit and warns", async () => {
    const zep = makeFakeZep();
    const warn = vi.fn();
    const tool = createZepRememberTool({
      client: asZep(zep),
    ...
    expect(result.stored).toBe(true);
    const sent = zep.graph.episode.add.mock.calls[0]![1].data as string;
