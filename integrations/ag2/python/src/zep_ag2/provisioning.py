...
``ZepMemoryManager``'s lazy call into these helpers (see
``ZepMemoryManager.ensure_user_and_thread``) is hot-path-wrapped and will
never raise into the memory paths (``get_memory_context``,
``enrich_system_message``, ``process_user_message``, ``add_messages``).
