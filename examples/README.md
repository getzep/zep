...
# Clean-environment / full static gate (structured installs, then static checks)
python3 examples/run_checks.py --install --mode static

# Print the plan without executing (add --install to include install steps)
...
Inventory / credential-gating / install-structure tests:
...
| `static` + `--install` | structured installs, then static checks | **No** |
...
Claude ingest honors `ZEP_EXAMPLE_USER_PREFIX`. Eve smoke sets `ZEP_DEMO_USER_ID` / `ZEP_COMPANY_GRAPH_ID` from the prefix.
Groups that mint their own UUIDs document that in notes (they do not consume the prefix env vars).
...
| `PROXY_API_KEY` / `VITE_ELEVENLABS_AGENT_ID` | `python/elevenlabs-zep-example` | **No — not live-covered** |
...
| | |
| --- | --- |
| **Install** | `pip install -r requirements.txt` (in `python`) |
...
| | |
| --- | --- |
| **Install** | `pip install -r ../requirements.txt` (in `python/graph_example`) |
...
| | |
| --- | --- |
| **Install** | `pip install -r ../requirements.txt` (in `python/chat_history`) |
...
| | |
| --- | --- |
| **Install** | `pip install -r requirements.txt` (in `python/chunking-example`) |
...
| | |
| --- | --- |
| **Install** | `pip install -r requirements.txt` (in `python/claude-prompt-caching-example`) |
...
| | |
| --- | --- |
| **Install** | `uv sync` (in `python/openai-agents-sdk`) |
| **Install notes** | Alternative without uv: pip install -e . |
...
| | |
| --- | --- |
| **Install** | `pip install -r requirements.txt` (in `python/agent-memory-full-example`) |
...
| | |
| --- | --- |
| **Install** | `pip install -r requirements.txt` (in `python/context-templates-example`) |
...
| | |
| --- | --- |
| **Install** | `pip install -r requirements.txt` (in `python/user-summary-instructions-example`) |
...
| | |
| --- | --- |
| **Install** | `pip install -r requirements.txt` (in `python/zep-quickstart-dashboard`) |
| **Run** | `python zep_ingest.py` · `streamlit run ui.py` |
| **Static test** | `python -m compileall -q .` |
| **Keys** | `ZEP_API_KEY`, `OPENAI_API_KEY` |
| **Notes** | streamlit run ui.py is interactive — not in the default gate. |
...
| | |
| --- | --- |
| **Install** | `pip install -r requirements.txt` (in `python/elevenlabs-zep-example/llm-proxy`) · `npm install` (in `python/elevenlabs-zep-example/react-app`) |
...
| **Keys** | `ZEP_API_KEY`, `OPENAI_API_KEY`, `PROXY_API_KEY`, `VITE_ELEVENLABS_AGENT_ID` |
...
| | |
| --- | --- |
| **Install** | `npm install` (in `typescript`) |
...
| | |
| --- | --- |
| **Install** | `npm install` (in `typescript/chunking-example`) |
...
| | |
| --- | --- |
| **Install** | `npm install` (in `typescript/langgraph`) |
...
| | |
| --- | --- |
| **Install** | `npm install` (in `typescript/eve`) |
| **Install notes** | Requires Node.js 24+. |
...
| | |
| --- | --- |
| **Install** | `yarn install` (in `typescript/zep-graph-visualization`) |
| **Run** | `yarn dev` |
...
| | |
| --- | --- |
| **Install** | `go mod download` (in `go/chunking-example`) |
