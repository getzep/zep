...
Loose snippets that exercise the published [`@getzep/zep-cloud`](https://www.npmjs.com/package/@getzep/zep-cloud) v3 API. Packaged apps under this tree (`eve`, `langgraph`, `chunking-example`, `zep-graph-visualization`) keep their own dependencies and are not part of this package.
...
| `npm run example:memory` | Thread messages + `getUserContext` memory retrieval |
...
- `example:graph:entity-types` calls `graph.setEntityTypes` without user/graph targets, which replaces the **project-level** ontology — use a disposable project key.
