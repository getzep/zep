...
| Dynamic instructions (turn-relevant recall) | `agent/instructions/zep-memory.ts` | `graph.getContext` (query = current utterance) |
| Hook (auto-persist) | `agent/hooks/zep-persist.ts` | `thread.addMessages`, `graph.warm` |
| Authored tool (user search) | `agent/tools/zep_search.ts` | `graph.getContext` (user graph) |
| Authored tool (company search) | `agent/tools/zep_search_company.ts` | `graph.getContext` (standalone graph) |
| Product tools steered by memory | `call_service_a` / `call_service_b` / `call_capability_x` | — |

Identity mapping:

- Eve `session.id` → one Zep thread UUID, which the demo keeps in a process map
- Eve `session.auth` principal (or `ZEP_DEMO_USER_KEY`) → one Zep user UUID and graph UUID
...
- `ZEP_DEMO_USER_KEY` — demo identity when Eve auth is not configured
- `ZEP_COMPANY_GRAPH_UUID` — the company graph UUID that `npm run seed:company` prints
...
3. Start a **new** Eve session with the same `ZEP_DEMO_USER_KEY` (or continue only after facts are visible), then ask: “Which service should you use for billing?” — the agent should prefer Service A from the turn’s Zep memory section and/or `zep_search`.
...
Eve HTTP message
  ...
  │     → ensure Zep user/thread + graph.warm
  │
  ├─ turn.started → instructions/zep-memory
  │     → peek stashed utterance (clear after search settles)
  │     → graph.getContext(graphUuid, query=truncated utterance ≤400 chars)
  ...
  │
  └─ tools
        zep_search / zep_search_company  (pinned graph UUIDs)
...
1. **Replace demo identity** — wire real auth (`Auth.js` / Clerk / etc.) on `agent/channels/eve.ts`, then resolve the user key from `ctx.session.auth.current.principalId` in `agent/lib/identity.ts` and store the Zep user UUID in your own database. Remove the `ZEP_DEMO_USER_KEY` fallback for multi-tenant production.
...
6. **Utterance stash is in-process** — `onMessage` and `turn.started` must run in the same Node process (`eve dev` / single instance). Multi-isolate hosts (e.g. Vercel Workflow) need an external stash. Create-session has no `sessionId` in `onMessage`; the demo queues by `ZEP_DEMO_USER_KEY` and rebinds on `session.started` (FIFO). Avoid concurrent new sessions that share one demo user key.
