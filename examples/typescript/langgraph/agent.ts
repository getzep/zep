...
// Tavily is optional — help and non-search turns work without TAVILY_API_KEY
const tools = buildTools(process.env);
const toolNode = new ToolNode(tools);

// Create a model; only bind tools when search is configured
...
function parseCommandLineArgs() {
  ...
  program
    ...
    .option(
      "--user-id <id>",
      "User ID to associate with the conversation (alternative format)",
    )
    .option("--threadId <id>", "Thread ID for the conversation")
    .option(
      "--thread-id <id>",
      "Thread ID for the conversation (alternative format)",
    )
...
if (isMainModule()) {
  ...
  const runCLI = async () => {
    ...
    if (tools.length === 0) {
      console.log(
        "Tavily search disabled (set TAVILY_API_KEY to enable web search)",
      );
    }
    if (args.userId) {
    ...
    const config = {
      configurable: { sessionId: args.threadId || "cli-session" },
    };
      ...
      try {
        ...
        try {
          ...
          if (previousMessages.length > 0) {
            ...
            console.log(
              `Loaded ${previousMessages.length} messages from previous conversation`,
            );
          }
        } catch (error) {
    ...
    const askQuestion = () => {
      rl.question("\nYou: ", async (input) => {
        ...
        try {
          ...
          const resultMessages = result.messages.filter(
