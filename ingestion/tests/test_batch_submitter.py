...
from tests.conftest import BATCH_UUID, GRAPH_UUID, make_batch_summary
...
class TestPaging:
    def test_351_episodes_two_add_calls(self, mock_zep):
        ...
        assert mock_zep.batch.add_items.call_count == 2
        first_items = mock_zep.batch.add_items.call_args_list[0].kwargs["items"]
        second_items = mock_zep.batch.add_items.call_args_list[1].kwargs["items"]
        assert len(first_items) == 350
        assert len(second_items) == 1
        mock_zep.batch.process.assert_called_once_with(BATCH_UUID)
    ...
    def test_item_payload_mapping(self, mock_zep):
        ...
        [item] = mock_zep.batch.add_items.call_args.kwargs["items"]
    ...
    def test_reference_time_loss_is_reported_as_a_warning(self, mock_zep):
        # v4 has no created_at on a batch item, so the caller must learn that
        # the value was dropped rather than assume that it was sent.
        result = BatchSubmitter(mock_zep).submit(
            [Episode(data="hello", created_at="2024-01-01T00:00:00Z")], DEST
        )
        [warning] = result.warnings
        assert "created_at" in warning
        assert "sequential" in warning

    def test_no_warning_without_a_reference_time(self, mock_zep):
        assert BatchSubmitter(mock_zep).submit(episodes(2), DEST).warnings == []

    def test_stream_order_preserved(self, mock_zep):
        BatchSubmitter(mock_zep, page_size=2).submit(episodes(5), DEST)
        seen = [
            item.data for c in mock_zep.batch.add_items.call_args_list for item in c.kwargs["items"]
        ]
...
class TestRetries:
    def test_rate_limited_add_is_retried(self, mock_zep):
        mock_zep.batch.add_items.side_effect = [ApiError(status_code=429), None]
    ...
    def test_server_error_add_is_not_retried(self, mock_zep):
        mock_zep.batch.add_items.side_effect = [ApiError(status_code=500), None]
        result = BatchSubmitter(mock_zep).submit(episodes(3), DEST)
        assert mock_zep.batch.add_items.call_count == 1
    ...
    def test_server_error_records_error_and_continues(self, mock_zep):
        ...
        mock_zep.batch.add_items.side_effect = add_side_effect
        result = BatchSubmitter(mock_zep, page_size=1, max_add_retries=2).submit(episodes(3), DEST)
        assert mock_zep.batch.add_items.call_count == 3  # no ambiguous 5xx retry; one try per page
        ...
        assert (
            ...
            == "batch.add_items failed: status=500, body=boom after 1 attempt(s)"
        )
    ...
    def test_add_error_reports_the_server_message_and_adds_nothing(self, mock_zep):
        ...
        mock_zep.batch.add_items.side_effect = ApiError(status_code=500, body="server error")
    ...
    def test_transient_process_failure_retried(self, mock_zep):
        mock_zep.batch.process.side_effect = [
            ...
            make_batch_summary(BATCH_UUID, "queued"),
        ]
...
class TestTransportErrors:
    ...
    def test_transport_error_on_add_still_processes_and_returns_batch_id(self, mock_zep):
        ...
        assert error.batch_id == BATCH_UUID
        assert (
            error.error == "batch.add_items failed: transport error ReadTimeout after 1 attempt(s)"
        )

    def test_connect_error_on_add_is_retried(self, mock_zep):
        mock_zep.batch.add_items.side_effect = [httpx.ConnectError("connection refused"), None]
        result = BatchSubmitter(mock_zep).submit(episodes(3), DEST)
        assert mock_zep.batch.add_items.call_count == 2
        assert result.add_errors == []
        assert result.items_submitted == 3
