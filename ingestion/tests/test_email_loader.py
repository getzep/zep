...
Three priorities this quarter. Sam Reyes owns delivery.
"""
...
def test_headers_and_body_in_data(eml_dir):
    ...
    assert "Dana Okafor <dana@example.com>" in episode.data
    assert "Kickoff priorities" in episode.data
    assert "Sam Reyes owns delivery." in episode.data
...
<body><p>Atlas ships in <b>May</b>.</p><p>Reply for details.</p></body></html>
"""
...
def test_html_only_email_falls_back_to_stripped_html(tmp_path):
    ...
    assert "Atlas ships in May." in episode.data
